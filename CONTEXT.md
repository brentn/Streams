...
**Ignored**:
A `{transactionId}` record marking one specific Transaction as suppressed from the surfaces that would otherwise show or total it — the Needs-categorization list, the ADR-0007 uncategorized-tributary bucket, a matched Flow/Transfer's Tributary drill-in list, `actualFlowMagnitude` (so Variance Alerts, Persistent Overrun Alerts, and Budget totals all inherit the exclusion automatically), and the Budgets list summary's average-income figure. Independent of `matchedTarget`: Ignored doesn't touch Direct Categorization, Categorization Rules, or matching at all — a Transaction can be both matched and Ignored at once, since Ignored governs suppression, not resolution. Stored in its own store, keyed by Transaction id, same reasoning as Direct Categorization (ADR-0018): a resync's full-record `put` would silently drop a field added to `Transaction` itself. Deliberately does **not** affect `balanceAtDate`/`balanceSeries`/`runningDryAlert` or any other reconstruction of the Account's actual/projected balance — those read every Transaction regardless of categorization state, since they reflect real bank-reported history (ADR-0001), not the user's categorization choices. Triggered from wherever the Transaction is already listed: a dedicated action next to "Assign" on an unmatched Transaction's row, or an action inside the assign-match picker for an already-matched one. One-way for v1 — no un-ignore affordance yet, mirroring Skipped Occurrence's (ADR-0014) precedent of shipping suppression before undo UI. See ADR-0019.
_Avoid_: Excluded, Dismissed, Hidden (too close to "hidden from one view," when this is closer to "shouldn't count anywhere"), Skipped/Skip (reserved for the distinct Skipped Occurrence concept, scoped to Flow occurrences, not Transactions)

### Sync

**Needs Reauthentication**:
An Account whose SimpleFIN sync is blocked because its stored credentials have been revoked or are otherwise invalid — signaled by a `con.auth`/`gen.auth` error code, an HTTP 403 on the accounts fetch, or a legacy (code-less) `errors` string matching `/auth required/i`, synthesized into a `legacy.auth-required` code at parse time so classification itself stays code-only. Never inferred from message text anywhere past that single parse-time boundary. A connection-level failure marks every Account under that connection this way at once, rather than being tracked as a separate Connection entity. Persists until a resync succeeds. The underlying SimpleFIN setup token stays valid throughout — it's the bank-side link that's broken, fixed by the user at the SimpleFIN Bridge's own site directly (Streams opens it in a new tab from the Reauthorize action, alongside a plain resync attempt). That first resync fires before the user's done anything at the Bridge, so it's expected to still observe the broken connection; `SyncCoordinator` then retries automatically the next time this tab becomes visible again, closing the loop without a redundant second click once the user's actually fixed it there. No new setup token needs to be pasted back into Streams.
...
**Outstanding**:
A recurring-kind Flow's most recently expected occurrence, once the Account's balance has synced past that occurrence's date with no Transaction ever matching it. Derived fresh from current Flow/Transaction/Account/Skipped-Occurrence state, never stored itself — clears the moment a match posts, automatically after 14 days past the occurrence's due date, or when the occurrence has been recorded as a Skipped Occurrence, whichever comes first. The first two clear on their own as time passes or a Transaction arrives; a Skip is the one deliberate, user-triggered exit, and — unlike the other two — persists rather than depending on anything else changing. Applies only to a recurring-kind Flow: a budget-kind Flow has no single occurrence to be Outstanding, and a Transfer isn't covered. Only the single latest occurrence is ever evaluated per Flow; a second occurrence going overdue while the first is unresolved signals a different problem, not accumulating lateness. The occurrence's own past-dated Tributary is excluded from rendering while Outstanding — only its same-day "Pending" stand-in shows, never both. See ADR-0012.
...
**Skipped Occurrence**:
A persisted record — `{flowId, occurrenceDate}` — that a specific recurring-kind Flow occurrence should never be treated as Outstanding, regardless of how much time passes. Scoped to the exact occurrence date it was recorded against: a later occurrence of the same recurring Flow goes Outstanding normally when its own time comes, unaffected by an earlier one's Skip. The only manually-triggered resolution Outstanding has — every other exit (a match, the 14-day grace period) is a pure function of current state; a Skip is the deliberate exception, storage and all. See ADR-0014.
