...
- **Persisted flag on the Flow record** — rejected at the time: every other alert in this codebase (`RunningDryAlert`, `VarianceAlert`, `budgetProgressStatus`) is a pure function of current state; a stored flag would need its own dismiss/cleanup machinery nothing else here has. Revisited and deliberately overturned by ADR-0014, which adds exactly that machinery — scoped to a single occurrence rather than the whole Flow — once a real need for manual dismissal emerged.
