...
12. As a user, I want to add a Step Change to a recurring Flow, so that I can change its expected amount from a specific date forward (e.g. a raise, a new rent price).
13. As a user, I want to add a Recurring Rule to a recurring Flow, so that its amount automatically adjusts by a delta on a specific calendar date every year (e.g. a seasonal utility increase), independent of its cadence.
...
- `Flow`: id, accountId, direction (`in`/`out`), kind (`recurring`/`budget`, fixed at creation), tolerance (`{ kind: 'percent' | 'fixed', value }`). Kind-specific fields:
  ...
  - `budget`: period (`month`/`year`), limit amount. No cadence, no Step Change/Recurring Rule timeline, no rollover — the limit resets cleanly at each period boundary.
...
- The Projection Engine is the module to test most heavily: nearly all the domain logic from this spec lives there — chronological Step Change / Recurring Rule application (recurring-kind Flow only), Transfer symmetry, Tolerance/Variance Alert evaluation (direction depending on kind/direction), Persistent Overrun Alert's 3-consecutive-period streak logic (budget-kind Flow only), Dry Floor/Running-Dry Alert evaluation within the Projection Horizon.
